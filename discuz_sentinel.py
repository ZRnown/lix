...
class DiscuzSentinel:
    ...
    def _clean_content(self, html_content: str) -> Tuple[str, List[str]]:
        ...
        for img in soup.find_all('img'):
            ...
            if src and 'smilies' not in src:
                # ==================== 新增代码开始 ====================
                # 修复：清洗 URL，去除 ? 后面的所有参数 (如 imageMogr2...)
                if '?' in src:
                    src = src.split('?')[0]
                # ==================== 新增代码结束 ====================

                full_url = urljoin(BASE_URL + '/', src)
                if full_url not in images: # 避免重复
    ...
    def _universal_upload_for_dingtalk(self, img_url: str) -> str:
        ...
        for attempt in range(3):  # 最多重试3次
                    ...
                    try:
                        ...
                        if data.get('code') == 200 and 'data' in data:
                            ...
                            if '非法图片文件' in error_msg:
                                self.logger.warning(f"[自建图床] 放弃上传 (文件非法/损坏): {img_url}")
                                return img_url  # 直接返回原图链接，不再重试
                            else:
            ...
            # ==================== 修改代码开始 ====================
            # 如果是 Connection aborted，通常是因为图床服务器主动断开
            # 这种情况下通常重试也无用，建议在最后一次尝试失败后返回原链接
            if attempt == 2:
                self.logger.warning("[自建图床] 多次上传失败，使用原图链接")
                return img_url
            # ==================== 修改代码结束 ====================
        ...
        # 上传失败，返回原链接
        self.logger.error("[自建图床] 上传失败，已达到最大重试次数，返回原链接")
        return img_url
